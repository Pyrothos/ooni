...
### 2.3.1.4 Submitting simple reports using a single API call

When you have single-entry reports, you can submit them by `POST`ing onto
the `/simple-report` endpoint:

`POST /simple-report`

This API only works for reports expressed in JSON format. The request body
should be the same as the one used for adding a entry to a report. The following
JSON document is an example body sent to the `/simple-report` API generated by
